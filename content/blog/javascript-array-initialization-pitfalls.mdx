...
// ✅ Using .fill().map()
const grid2 = Array(3).fill(0).map(() => Array(3).fill(0));
...
var generateMatrix = function(n) {
    // Start with the correct, safe 2D array initialization
    const matrix = Array.from({ length: n }, () => Array(n).fill(0));
    ...
    while (top <= bottom && left <= right) {
        ...
        // Fill bottom row from right to left (if it exists)
        if (top <= bottom) {
        ...
        // Fill left column from bottom to top (if it exists)
        if (left <= right) {
...
> **A:** The first fills all rows with a reference to the *same* array. The second uses `.map` to create a *new, independent* array for each row.
